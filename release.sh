...
# Check if local main has diverged from remote
if ! git diff --quiet origin/main..main || ! git diff --quiet main..origin/main; then
  echo "⚠️ Local main is out of sync with origin/main. Attempting rebase..."
  git pull --rebase origin main || abort "Failed to rebase local main with origin"
fi
